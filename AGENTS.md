...
- `programs/<name>/` — a single program/tool, as `configuration.nix` (system-level) and/or `home.nix` (home-manager), depending on what the program needs. These are imported either directly from a host (for programs any host should have, e.g. `programs/claude-code`, `programs/1password`) or from a desktop module (for programs tied to that desktop but reusable independently of it, e.g. `programs/kitty`, `programs/thunar`).
