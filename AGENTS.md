...
- `desktops/<name>/` — a desktop environment as a paired unit: `configuration.nix` (system-level: session packages, `programs.hyprland`, `programs.noctalia`, etc.) and `home.nix` (home-manager: per-user settings for that desktop). The system module's imports list ends with `home-manager.users.sander.imports = [ ./home.nix ... ]`, tying the two halves together so importing the desktop's system module automatically pulls in the matching home-manager config. This keeps system and home-manager config from drifting out of sync — see the `desktops/noctalia/configuration.nix` example, which also pulls in `programs/kitty/home.nix` this way. Config for a program that's tightly coupled to the desktop itself (e.g. Hyprland's own config, which requires Noctalia's Lua module and references its layer namespaces directly) lives inline in the desktop's own files rather than as a separate `programs/` module — see `desktops/noctalia/home.nix` and `desktops/noctalia/config/hyprland.lua`.
- `programs/<name>/` — a single program/tool, as `configuration.nix` (system-level) and/or `home.nix` (home-manager), depending on what the program needs. These are imported either directly from a host (for programs any host should have, e.g. `programs/claude-desktop`, `programs/1password`) or from a desktop module (for programs tied to that desktop but reusable independently of it, e.g. `programs/kitty`, `programs/thunar`).
